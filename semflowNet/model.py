...
import ot  # 【新增】引入 Python Optimal Transport 库
...
class FlowDriveNet(nn.Module):
    ...
    def forward(self, x_t, t, semantic_emb):
        # 这个前向传播专供旧代码调用或特定测试，实际在生成中我们会分步调用
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch in dataloader:
            ...
            if is_ae_phase:
                ...
                # ====================================================
                # Phase 2: 训练 Latent FlowNet (集成 Minibatch OT)
                # ====================================================
                with torch.no_grad():
                ...
                # ----------------------------------------------------
                # 【核心前沿优化：Minibatch Optimal Transport 配对】
                # ----------------------------------------------------
                # 1. 计算 Batch 内噪声 z_0 和真实数据 z_1 的欧式距离平方矩阵
                ...
                # 3. 求解最优传输计划 (Earth Mover's Distance)
                pi = ot.emd(a, b, cost_matrix)
                
                # 4. 根据最优分配矩阵，重新排列 z_0，实现“就近匹配”
                # pi 的每一行是一个 one-hot 形式的分配，argmax 提取目标索引
                ...
                # Rectified Flow 线性插值与目标速度场 (使用配对后的 z_0)
